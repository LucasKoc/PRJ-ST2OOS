...
from api.models.enrollments import EnrollmentModel, EnrollmentModelDB
from api.services.courses_services import get_course
from api.services.students_services import get_student
from api.services.enrollments_services import create_enrollment, get_enrollments, get_student_enrollments, \
    verify_enrollment_existence

router = APIRouter()

@router.post("/", response_model=EnrollmentModel, summary="Create a new enrollment", description="Create a new enrollment", response_description="Enrollment created", status_code=201, responses={400: {"description": "Bad request"}, 404: {"description": "Student or course not found"}})
...
@router.get("/{student_id}", response_model=List[EnrollmentModel], summary="Get student enrollments", description="Get student enrollments", response_description="Student enrollments retrieved", status_code=200, responses={404: {"description": "Student or Student's enrollments not found"}, 400: {"description": "Bad request"}})
